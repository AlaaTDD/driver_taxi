...
export async function deleteBonusRule(id: string) {
  ...
  // Capture old_data for the audit log before deleting.
  const { data: oldRow } = await supabase
    .from("bonus_rules")
    .select("name, name_ar, trigger_type, threshold, bonus_amount")
    .eq("id", id)
    .single();
...
export async function updateBonusRule(id: string, formData: FormData) {
  const guard = await requireAdmin();
  if (guard instanceof Response) return { error: "غير مصرح لك بإجراء هذا العمل" };
  ...
  // Capture old_data for audit log.
  const { data: oldRow } = await supabase
    .from("bonus_rules")
    .select("name, name_ar, trigger_type, threshold, bonus_amount, is_active")
    .eq("id", id)
    .single();
  ...
  const { error } = await supabase.from("bonus_rules").update(data).eq("id", id);
  if (error) return { error: safeErrorMessage(error) };
